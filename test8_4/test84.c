...
void do_limit(int limit,const char *limit_string,struct rlimit *rlim){
    ...
    printf("%15s=%2d: cur =%20lu,  max=%20lu\n",limit_string,limit,rlim->rlim_cur,rlim->rlim_max);
}

void print_limits(){
    struct rlimit rlim;
    do_limit(RLIMIT_CPU,"RLTMIT_CPU",&rlim);
...
void print_rusage(int who){
    ...
    printf("ru_utime.tv_sec,ru_utime.tv_usec = %4d  %4d(urer time used)\n",
...
int main(){
    ...
    printf("/nBefore Modification, this is RLIMIT_CORE:\n");
    do_limit(RLIMIT_CORE,"RLIMIT_CORE",&rlim);
    ...
    for(i=0;i<nchildren;i++){
        ...
        if(pid == 0){
            printf("\nIn child pid %d this RLIMIT_CORE:\n",(int)getpid());
            do_limit(RLIMIT_CORE,"RLIMIT_CORE",&rlim);
            fflush(stdout);
            sleep(3);
            exit(EXIT_SUCCESS);
